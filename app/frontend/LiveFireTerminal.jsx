...
// LIVEFIRE TERMINAL â€” LiveFire Edition
...
export default function LiveFire() {
  ...
  const [log, setLog] = useState(['LIVEFIRE LIVEFIRE TERMINAL v1.0 â€” TYPE HELP <GO>']);
  ...
  const run = async (kind) => {
    ...
    pushLog(`${kind.toUpperCase()} ${profile.toUpperCase()} â€” SENDING`);
    try {
      ...
      if (kind === 'tournament') {
        pushLog(`TOURNAMENT ESCALATION ${j.escalation?.map(x=>(x*100).toFixed(1)+'%').join(' â†’ ')}  RED ADV ${(j.red_advantage*100).toFixed(1)}%`);
      } else {
  ...
  const onCmd = (e) => {
    ...
    else if (c.startsWith('PROFILE')) { const p=c.split(' ')[1]?.toLowerCase(); if(p) { setProfile(p); pushLog(`PROFILE â†’ ${p}`);} }
    else if (c === 'EXPORT') window.open(`${API}/api/ledger/export`, '_blank');
    else pushLog(`UNKNOWN: ${c} â€” TYPE HELP`);
  };
  ...
  return (
    <div className="min-h-screen bg-black text-amber-500 font-mono text-[12px] leading-[1.35] flex flex-col select-none" style={{ fontFamily: 'JetBrains Mono, ui-monospace, SFMono-Regular, Menlo, monospace' }}>
      ...
      <div className="h-[28px] bg-[#FF8C00] text-black flex items-center px-2 gap-3 font-black tracking-widest text-[11px] shrink-0">
        ...
        <span className="opacity-60">|</span>
        <span>{health ? `${health.vectors} VECTORS â€¢ ${health.rail_profiles?.join('/')}` : 'CONNECTING...'}</span>
      ...
      <div className="h-[22px] bg-[#1a1a1a] border-y border-[#333] flex items-center px-1 gap-[2px] text-[10px] shrink-0">
        ...
        <span className="ml-auto text-[#666]">VECTORS {vectors.length} â€¢ LEDGER {health?.ledger_campaigns || 0} â€¢ PROFILE {profile} â€¢ TYPE HELP &lt;GO&gt;</span>
      </div>
      ...
      <div className="flex-1 grid grid-cols-12 gap-[1px] bg-[#222] p-[1px] min-h-0">
        ...
        <div className="col-span-8 bg-black flex flex-col min-h-0">
          <div className="h-[20px] bg-[#111] border-b border-[#333] flex items-center px-2 text-[10px] tracking-widest font-bold">
            <span className="text-[#FF8C00]">BLOTTER â€” ROBUSTNESS LEDGER</span>
            <span className="ml-auto text-[#666]">{cols.length} CAMPAIGNS â€¢ LAST 24 SHOWN â€¢ CLICK ROW â†’ DETAIL</span>
          </div>
          ...
          <div className="flex-1 overflow-auto divide-y divide-[#1a1a1a]">
            {cols.length === 0 ? (
              <div className="p-6 text-center text-[#444]">NO CAMPAIGNS â€” HIT F1 RUN</div>
            ) : cols.map((c, i) => (
              <div key={c.id || i} onClick={() => setSelected(c)} className={`grid grid-cols-12 gap-0 px-2 py-[4px] hover:bg-[#111] cursor-pointer text-[11px] ${selected?.id===c.id ? 'bg-[#1a1200] !text-[#FFB000]' : i%2===0 ? 'bg-black' : 'bg-[#080808]'}`}>
                ...
                <span className="col-span-4 truncate text-[#777]">{c.evasion_notes || 'â€”'}</span>
              </div>
            ))}
          </div>
          <div className="h-[20px] bg-[#111] border-t border-[#333] flex items-center px-2 gap-2 text-[10px]">
            ...
            <button onClick={() => run('tournament')} disabled={running} className="bg-black border border-[#FF8C00] text-[#FF8C00] px-3 py-[2px] font-black disabled:opacity-50">F2 TOURNAMENT (10Ã—)</button>
            ...
            <span className="ml-auto text-[#555]">DETECT LAT {health?.detect_latency_ms ? `${health.detect_latency_ms.p50_ms}/${health.detect_latency_ms.p99_ms}ms` : 'â€”'} â€¢ LLM {health?.llm_configured ? 'ON' : 'OFF'}</span>
          </div>
        </div>
        ...
        <div className="col-span-4 flex flex-col gap-[1px] bg-[#222] min-h-0">
          ...
          <div className="bg-black flex flex-col" style={{ flex: 1 }}>
            <div className="h-[20px] bg-[#111] border-b border-[#333] px-2 flex items-center text-[10px] font-bold tracking-widest">
              <span className="text-[#FF8C00]">ROUND HISTORY â€” BLUE EVOLUTION</span>
            </div>
            <div className="overflow-auto flex-1 divide-y divide-[#1a1a1a]">
              ...
              {rounds.length===0 && <div className="p-4 text-center text-[#444] text-[11px]">NO ROUNDS â€” RUN ONE</div>}
            </div>
          </div>
          ...
          <div className="bg-black border-t border-[#222]" style={{ minHeight: selected ? 140 : 40 }}>
            <div className="h-[20px] bg-[#FF8C00] text-black px-2 flex items-center text-[10px] font-black tracking-widest">
              DETAIL {selected ? `â€” ${selected.vector}` : 'â€” SELECT ROW'}
            </div>
            {selected ? (
              <div className="p-2 text-[11px] leading-[1.4]">
                ...
                <div className="mt-2 p-2 bg-[#0a0a0a] border border-[#222] text-[#999] whitespace-pre-wrap break-words max-h-[80px] overflow-auto">{selected.evasion_notes || 'â€”'}{selected.plan_json ? `\nPLAN ${selected.plan_json.slice(0, 400)}` : ''}</div>
              </div>
      ...
      <div className="h-[26px] bg-black border-t border-[#FF8C00] flex items-center px-2 gap-2 shrink-0">
        ...
        <input
          ...
          onKeyDown={onCmd}
          placeholder="TYPE COMMAND â€” HELP | RUN | TOUR | PROFILE | EXPORT  â€” ENTER <GO>"
        ...
        <span className="text-[#444] text-[10px] hidden sm:inline">LIVEFIRE TERMINAL â€¢ LIVEFIRE â€¢ CODE0710</span>
      </div>
    </div>
  );
}
